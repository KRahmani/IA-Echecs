...
// Fonction vérifiant si le score a chuté et si on doit apprendre ou pas le score de la position « P »
void checkLearning() 
{
...
void think(int output)
{
	...
	if (stop_search) {
		...
		while (ply)
			takeback();
		checkLearning(); // Sortie de think() sur timeout = réflexion en temps limité(long jump)
	...
	checkLearning(); // Sortie de think() lorsque maxdepth est atteint, réflexion en profondeur fixe.
